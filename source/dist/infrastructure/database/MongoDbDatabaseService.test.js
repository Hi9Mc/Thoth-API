...
describe('MongoDbDatabaseService', () => {
    ...
    beforeEach(() => {
        // Clear all mocks
        jest.clearAllMocks();
        db = new MongoDbDatabaseService_1.MongoDbDatabaseService('mongodb://test', 'test-db', 'test-collection');
        // Get mock instances
    ...
    describe('update', () => {
        it('should update an existing object', async () => {
            ...
            expect(mockCollection.replaceOne).toHaveBeenCalledWith({ _id: 'p1#typeA#c1#1' }, { ...updatedObj, _id: 'p1#typeA#c1#1' });
        });
        it('should throw error when object not found', async () => {
            mockCollection.findOne.mockResolvedValue(null);
            const nonExistentObj = { tenantId: 'p999', resourceType: 'typeX', resourceId: 'c999', version: 1, name: 'NonExistent' };
    ...
    describe('search', () => {
        ...
        it('should handle LIKE operator', async () => {
            ...
            const condition = {
                logic: IDatabaseService_1.SearchLogicalOperator.AND,
                conditions: [
                    { key: 'name', value: 'lph', operator: IDatabaseService_1.SearchConditionOperator.LIKE },
                ],
            };
            const pagination = { page: 1, limit: 10 };
            const { results, total } = await db.search(condition, pagination);
        ...
        it('should handle IN operator', async () => {
            ...
            const condition = {
                logic: IDatabaseService_1.SearchLogicalOperator.AND,
                conditions: [
                    { key: 'name', value: ['Alpha', 'Gamma'], operator: IDatabaseService_1.SearchConditionOperator.IN },
                ],
            };
            const pagination = { page: 1, limit: 10 };
            const { results, total } = await db.search(condition, pagination);
    ...
    describe('exists', () => {
        ...
        it('should return false for non-existing items', async () => {
            ...
            const condition = {
                logic: IDatabaseService_1.SearchLogicalOperator.AND,
                conditions: [
                    { key: 'tenantId', value: 'nonexistent', operator: IDatabaseService_1.SearchConditionOperator.EQUALS },
                ],
            };
            const result = await db.exists(condition);
            expect(result).toBe(false);
    ...
    describe('singleton pattern', () => {
        it('should return same instance for getInstance', () => {
            const instance1 = MongoDbDatabaseService_1.MongoDbDatabaseService.getInstance();
            const instance2 = MongoDbDatabaseService_1.MongoDbDatabaseService.getInstance();
            expect(instance1).toBe(instance2);
        });
        it('should return project-specific instances for getInstanceByTenantId', () => {
            const instance1 = MongoDbDatabaseService_1.MongoDbDatabaseService.getInstanceByTenantId('project1');
            const instance2 = MongoDbDatabaseService_1.MongoDbDatabaseService.getInstanceByTenantId('project1');
            const instance3 = MongoDbDatabaseService_1.MongoDbDatabaseService.getInstanceByTenantId('project2');
            expect(instance1).toBe(instance2);
