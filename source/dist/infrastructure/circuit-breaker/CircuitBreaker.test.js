...
describe('CircuitBreaker', () => {
    ...
    describe('CLOSED state', () => {
        it('should execute operation successfully when closed', async () => {
            ...
            expect(circuitBreaker.getMetrics().state).toBe(CircuitBreaker_1.CircuitBreakerState.CLOSED);
        });
        it('should transition to OPEN state after threshold failures', async () => {
            ...
            expect(circuitBreaker.getMetrics().state).toBe(CircuitBreaker_1.CircuitBreakerState.OPEN);
    ...
    describe('OPEN state', () => {
        ...
        it('should reject operations immediately when open', async () => {
            const operation = jest.fn().mockResolvedValue('success');
            await expect(circuitBreaker.execute(operation)).rejects.toThrow(CircuitBreaker_1.CircuitBreakerError);
